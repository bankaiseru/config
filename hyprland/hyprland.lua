...
hl.on("hyprland.start", function () 
    hl.exec_cmd("qs -p $HOME/code/icevoxe/shell/shell.qml") -- IceShell
end)
...
hl.bind(mainMod .. " + U", hl.dsp.exec_cmd("unityhub")) -- Code Editor
hl.bind(mainMod .. " + O", hl.dsp.exec_cmd(notes)) -- Notes
hl.bind(mainMod .. " + U", hl.dsp.exec_cmd("virtualbox")) -- Virtual Box
hl.bind(mainMod .. " + G", hl.dsp.exec_cmd("gimp"))

hl.bind(mainMod .. " + B", hl.dsp.exec_cmd(browser)) -- Browser
hl.bind(mainMod .. " + T", hl.dsp.exec_cmd(messenger)) -- Messenger
...
hl.bind(mainAlt .. " + M", hl.dsp.exec_cmd("prismlauncher")) -- Minecraft
...
hl.bind(mainMod .. " + P", hl.dsp.exec_cmd("happ")) --hl.dsp.window.pseudo())
...
-- System
hl.bind(mainShift .. " + Q ", hl.dsp.exec_cmd("systemctl suspend"))
...
local mainAll = { mainMod, mainAlt }

for mod_idx, modifier in ipairs(mainAll) do
    local workspace_offset = (mod_idx - 1) * 10

    for i = 1, 10 do
        local key = i % 10
        local ws_num = i + workspace_offset

        hl.bind(modifier .. " + " .. key,           hl.dsp.focus({ workspace = ws_num }))
        hl.bind(modifier .. " + SHIFT + " .. key,   hl.dsp.window.move({ workspace = ws_num }))
    end
end
